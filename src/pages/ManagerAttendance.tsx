    ...
    <div className="space-y-6">
      ...
      <Dialog open={bulkLateDialog} onOpenChange={setBulkLateDialog}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <DialogFooter><Button variant="outline" onClick={() => setBulkLateDialog(false)}>{t('common.cancel')}</Button><Button onClick={() => { const valid = bulkRows.filter(r => r.managerId); if (!valid.length) { toast.error(t('attendance.addValidRow')); return; } bulkLateMut.mutate(valid.map(r => ({ managerId: r.managerId, date: bulkShared ? bulkSharedDate : r.date, isJustified: r.isJustified, reason: r.isJustified ? r.reason : undefined, period: r.period ?? 10 }))); }}>{t('attendance.addCountLates', { count: bulkRows.filter(r => r.managerId).length })}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
