    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="flex flex-wrap gap-2">
                ...
                {teacher.subjectIds.length === 0 && <p className="text-muted-foreground">{t('common.noneAssigned')}</p>}
              </CardContent>
            </Card>
            <Card>
                    ...
                    <div key={a.classId} className="flex items-center gap-2 flex-wrap">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {a.subjectIds.map(sid => (
                          <Badge key={sid} variant="secondary" className="text-xs">{subjectsRes?.data?.find(s => s.id === sid)?.name || sid}</Badge>
                        ))}
                      </div>
                    </div>
                  );
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{t('teachers.editTeacher')}</DialogTitle></DialogHeader>
